...
def process_csv(input_file, output_file):
    ...
    print(f"  Header: {len(header_fields)} cols, SKU={idx_sku}, Price={idx_price}")
        ...
        if is_valid_handle(handle_candidate):
            ...
            if title_candidate and not title_candidate.startswith('"') and not title_candidate.startswith('<'):
                ...
                if current and handle_candidate == current['handle']:
                    # Try to parse as CSV for SKU/price extraction
                    try:
                        rows = list(csv.reader(StringIO(unwrapped)))
                        if rows and rows[0]:
                            fields = rows[0]
                            sku = fields[idx_sku].strip().strip("'") if len(fields) > idx_sku else ''
                            price = fields[idx_price].strip() if len(fields) > idx_price else ''
                            if sku or price:
